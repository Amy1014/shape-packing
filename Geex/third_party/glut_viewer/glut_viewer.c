...
#include <Geex/third_party/AntTweakBar/include/AntTweakBar.h>
#endif
...
static int nb_caps = 12 ;

static GLfloat params[6] = {
    0.5f,  // HDR_EXPOSURE
...
static int nb_params = 6;

static char* title = "g33>|< Viewer" ;

static int skybox_tex = 0 ;
static int background_tex = 0 ; 
...
static float cur_rot[4];
static float cur_rot_light[4] ;
static float zoom = 1.0 ;
static float cur_xlat[3] = {0.0, 0.0, 0.0} ;
static enum { NONE, ROTATE, PAN, ZOOM} mode = NONE ;
static float xmin=-1, ymin=-1, zmin=-1, xmax=1, ymax=1, zmax=1 ;
static float roi_radius = 1.4 ;
...
static void save_picked_ray(int x_in, int y_in) {
    ...
    if(transform_saved) {
        ...
        /*
         * Get the correct value of Z (since even 2D mode uses perspective xform, if we do not do
...
static GLboolean call_mouse_func(int x, int y, int button, enum GlutViewerEvent event) {
    ...
    save_picked_ray(x,y) ;
    if(mouse_func == NULL) { return result ; }
    result = mouse_func(mx, my, button, event) ;   
    glutPostRedisplay() ;
    return result ;
}
...
#ifdef WITH_ANTTWEAKBAR
    if(glut_viewer_is_enabled(GLUT_VIEWER_TWEAKBARS) && TwEventMouseButtonGLUT(button, state, x, y)) {
        glutPostRedisplay() ;
        return ;
    }
#endif    
...
#ifdef WITH_ANTTWEAKBAR
    if(glut_viewer_is_enabled(GLUT_VIEWER_TWEAKBARS) && TwEventMouseMotionGLUT(x, y)) {
        glutPostRedisplay() ;
        return ;
    }
#endif    
...
static void motion(int x, int y) {
    ...
    case PAN: {
        if(!glut_viewer_is_enabled(GLUT_VIEWER_ROTATE_LIGHT)) {
            float delta_x = (float)(last_x - x) / W ;
            float delta_y = (float)(y - last_y) / H ;
            cur_xlat[0] -= 2.0 * delta_x ;
            cur_xlat[1] -= 2.0 * delta_y ;
        }
    } break ;
    case ZOOM: {
       if(!glut_viewer_is_enabled(GLUT_VIEWER_ROTATE_LIGHT)) {
           zoom = zoom * (1.0 + (((float) (y - last_y)) / H));
       }
    } break ;
    default:
        break ;
	   ...
	   glut_viewer_printf(" --- %s help [%4d FPS ] --- ", title, glut_viewer_fps()) ;
        ...
        for(i=0; i<256; i++) {
            ...
            if(key_description[i] != NULL) {
                if(toggle[i] != NULL) {
                    glut_viewer_printf("%c:toggle %s (%s)\n", (char)i, key_description[i], *toggle[i] ? "on" : "off") ;
                }
                if(key_func[i] != NULL) {
        ...
        if(glut_viewer_is_enabled(GLUT_VIEWER_HDR)) {
            glut_viewer_printf("Exposure: [<]%f[>]", glut_viewer_get_float(GLUT_VIEWER_HDR_EXPOSURE)) ;
	}
    }

    if(glut_viewer_is_enabled(GLUT_VIEWER_TWEAKBARS) && overlay_func != NULL) {
        overlay_func() ;
    }
#ifdef WITH_ANTTWEAKBAR
...
static float to_degrees(float x) { return x * 360.0 / (2.0 * M_PI) ; }
static float to_radians(float x) { return x * 2.0 * M_PI / 360.0  ; }
...
static void actually_render_display(double offset){
    GLfloat m[4][4];
    static GLfloat light_diffuse[] = {1.0, 1.0, 1.0, 1.0};  /* White diffuse light. */
    static GLfloat light_position[] = {1.0, 1.0, 1.0, 0.0};  /* Infinite light location. */
    
    float camera_aperture = 9.0 ;  // field of view of the larger dimension in degrees
    float zNear = 1.0 ;             // near clipping plane
    float zFar = 10.0 ;             // far clipping plane
    float zScreen = 5.0;            // screen projection plane
    float eye_offset = offset;      // half of the distance between the eyes, if in stereo mode

    float aspect = (double)glut_viewer_W / (double)glut_viewer_H ;       // aspect ratio of the window
    float vue_max_size = zScreen * tan(to_radians(camera_aperture)/2) ;  // half the width of the screen 
                                                                         // from the central point of view
    float vue_shift = eye_offset * zNear / zScreen;                      // shift of the vue from the current point of view
    ...
    // apply zoom
    if(glut_viewer_is_enabled(GLUT_VIEWER_3D)) {
	glScalef(zoom*2.0, zoom*2.0, zoom*2.0) ; 
    } else {
	glScalef(zoom*1.3, zoom*1.3, zoom*1.3) ; 
    }
    ...
    glScalef(1.0 / roi_radius, 1.0 / roi_radius, 1.0 / roi_radius) ;
    glTranslatef(
        -0.5 * (xmin + xmax),
        -0.5 * (ymin + ymax),
        -0.5 * (zmin + zmax)
...
static void set_key_description(char key, const char* description) {
    if(key_description[key] != NULL) {
        printf(
            "Warning: overriding key %c with %s (was %s)\n", 
            key, description ? description : "NULL", key_description[key]
        ) ;
    }
    free(key_description[key]) ;
    if(description == NULL) {
        key_description[key] = NULL ;
    } else {
        key_description[key] = malloc(strlen(description) + 1) ;
        strcpy(key_description[key], description) ;
    }
}
...
static void cleanup() {
    int i ; // variable must be defined before function calling
    printf("Terminating GlutViewer\n") ;
#ifdef WITH_ANTTWEAKBAR
    TwTerminate() ;
#endif
    for(i=0; i<256; ++i) {
...
void glut_viewer_main_loop(int argc, char** argv) {
    ...
    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB
        | ( glut_viewer_is_enabled(GLUT_VIEWER_HDR) ? 0 : GLUT_DEPTH)
        | ( glut_viewer_is_enabled(GLUT_VIEWER_STEREOSCOPIC_DISPLAY) ? GLUT_STEREO : 0 ) );
...
static void copy_image_to_clipboard() {
    int w = glut_viewer_W ; int h = glut_viewer_H ;
    int row_len = w * 3 ;  
    int image_size, size ;
    HANDLE handle;  
...
/* #define printH(x) printf("%s = 0x%lx\n", #x, header.x) */
#define printH(x) 
...
static GLboolean glut_viewer_load_image_BMP(
   ...
   *bpp = header.sBitCount / 8 ;
   ...
   for(i=0; i<(int)*height; i++) {
      fread(pixels + i*rowlen, rowlen, 1, f) ;
      fread(padding, rowlen % 4, 1, f) ; /* BMP rows are padded to 4-bytes multiples */
   }
...
void glTexImage2Dfile(const char* filename) {
    ...
    case 1:
        gluBuild2DMipmaps(
            GL_TEXTURE_2D, GL_LUMINANCE, width, height, GL_LUMINANCE, GL_UNSIGNED_BYTE, pixels
        ) ;
        break ;
    case 3:
        gluBuild2DMipmaps(
            GL_TEXTURE_2D, GL_RGB, width, height, GL_RGB, GL_UNSIGNED_BYTE, pixels
        ) ;
        break ;
    case 4:
        gluBuild2DMipmaps(
            GL_TEXTURE_2D, GL_RGBA, width, height, GL_RGBA, GL_UNSIGNED_BYTE, pixels
        ) ;
        break ;
    default:
        assert(0) ;
