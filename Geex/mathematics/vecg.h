...
namespace Geex {
	...
	template <class T> inline vec3g<T> normalize(const vec3g<T>& v) 
	{
		T l = length(v);
		return vec3g<T>(v.x/l, v.y/l, v.z/l);
	}
...
// -------------------------- vec4 -----------------------------------------------------------------------------------------------------------------------------------
	...
	template <class T> inline vec4g<T> normalize(const vec4g<T>& v) 
	{
		T l = length(v);
		return vec4g<T>(v.x/l, v.y/l, v.z/l, v.w);
	}
