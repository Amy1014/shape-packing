...
class SphericalBounder
{
	...
	typedef CGAL::Straight_skeleton_2<Geex::K> Ss;
	typedef boost::shared_ptr<Ss> SsPrt;
public:
	...
	//~SphericalBounder();
	void setPolygon(const vector<Segment_3>& aPolygon);
	...
	void genMarginBounder(unsigned int nbSamping); 
	void genMarginBounder(double sf);
	void genMarginBounder( double sf, unsigned int nbSampling);
	void genInteriorMarginBounder(unsigned int nbSampling);
	void genInteriorMarginBounder(unsigned int nbSapmling, const vec3& norm);
	const vector<Weighted_point>& getMarginBounder() const { return msu;}
	static vector<Weighted_point>* genInteriorMarginBounder(const vector<Segment_3>& plygn, unsigned int nbSampling);
	void drawPolygon();
	void drawMedialAxis();
private:
	...
	static inline Vector_2 normalize(const Vector_2& v)
	{
		return v / sqrt(v.squared_length());
	}
	static inline Vector_3 normalize(const Vector_3& v)
	{
		return v / sqrt(v.squared_length());
	}
private:
