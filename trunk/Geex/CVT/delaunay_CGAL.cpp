...
//      class DelaunayBase : public CGAL::Triangulation_hierarchy_3<TRI> {
    ...
    Delaunay_CGAL::Delaunay_CGAL(bool use_spatial_sort) : use_spatial_sort_(use_spatial_sort) {
    ...
    void Delaunay_CGAL::set_vertices(unsigned int n, const double* xyz) {
        ...
        if(use_spatial_sort_) {
            ...
            for(unsigned int i = 0; i < n; i++) {
                ...
                impl_->sorted_points_[i].pt = Implementation::Point(
            ...
            CGAL::CVT_spatial_sort_3(
                impl_->sorted_points_.begin(), impl_->sorted_points_.end(), MyCGALStuff::K()
            ) ;
	...
	void Delaunay_CGAL::set_cells_id()	{
		int cell_start = 0;
	...
	unsigned int Delaunay_CGAL::get_number_of_finite_vertices() const{return (unsigned int)impl_->number_of_vertices(); }
	unsigned int Delaunay_CGAL::get_number_of_finite_cells() const{return (unsigned int)impl_->number_of_finite_cells(); }
    ...
    void Delaunay_CGAL::get_voronoi_cell(unsigned int iv, VoronoiCell& result, bool geometry, bool use_cell_id) {
        result.clear() ;
                                ...
                                if(geometry) {
									if (!cir->is_circumcenter_computed()) {
										...
										if(c_is_infinite) {
											...
											cir->self_circumcenter() = MyCGALStuff::to_geex(impl_->dual(cir));;
										}
									}
									if (use_cell_id) {
										if (!cell_id_tag_)
