                        ...
                        if(facet_vertices_[i].size() > 0) {
                            ...
                            if(traverse_cell_) {
                                ...
                                for(unsigned int k=0; k<facet_vertices_[i].size(); k++) {
                                    action.vertex(facet_vertices_[i][k], facet_vertices_[i][k].cell_id) ;
                                }
                                action.end_facet() ;
                            }
                        }
    ...
    protected:
        ...
        template <class ACTION> void apply_action_to_cell_facet(unsigned int fi, const ACTION& action) {
            ...
            for(unsigned int i = cell_.facet_begin(fi); i<cell_.facet_end(fi); i++) {
                ...
                k.insert(f+1) ;
                if(geometry_) {
                    action.vertex(Vertex(cell_.vertex(i),k), cell_.cell_id(i)) ;
                } else {
                    action.vertex(Vertex(vec3(0,0,0),k), cell_.cell_id(i)) ;
                }
            }
            action.end_facet() ;
        }
        ...
        template <class ACTION> void get_contour(
            ...
            if(!is_corner(from)) {
                ...
                if(traverse_cell_) {
                    ...
                    do {
                        action.vertex(it->second, it->second.cell_id) ;
            ...
            if(facet_vertices_[f].size() > 0) {
                ...
                if(traverse_cell_) {
                    ...
                    for(unsigned int k=0; k<facet_vertices_[f].size(); k++) {
                        action.vertex(facet_vertices_[f][k], facet_vertices_[f][k].cell_id) ;
                    }
                    action.end_facet() ;
                }
            }
